...
- **Plan readiness is a separate axis from plan status** (ruled in `task:t131`). On a `kind: plan` entity, `status:` carries only the toolkit lifecycle vocabulary (`active`, `archived`, `complete`, `draft`, `retired`, `superseded`); the execution-readiness verdict lives in a project-local `readiness:` field (`ready-with-caveats`, `not-ready`). The `readiness:` field is inert — nothing in `science` validates or surfaces it (filed upstream as `fb-2026-07-26-011`) — so it must not be the only place a load-bearing ruling is recorded; state it in the plan body too. Do **not** flatten a readiness verdict into `status:`, and do not use `verdict:` for it (that key is parsed as a structured mapping).
- **Annotation markers: lift the terminal ones, leave the pending ones warning** (ruled in `task:t143`). The four tokens already split on permanence, and the toolkit's own default severities encode it: `[SPECULATION]` and `[INACCESSIBLE]` are terminal (author conjecture; structurally closed source) and default to *info*, while `[UNVERIFIED]` and `[MISSING_CITATION]` mean "checkable, not yet checked" and default to *warn*. Do not invent a parallel vocabulary. Adjudicate a terminal marker with `uv run science annotate lift-tokens --root .` **without `--remove`**: the token stays visible in the prose, the warning clears, and the ruling is recorded as a W3C annotation in a `<entity>.anno.trig` sidecar whose content-anchored selector re-fires the warning if the sentence is later edited. A standing `[UNVERIFIED]` is a real backlog item and must keep warning — never lift one, and never clear one by asserting the claim. Note `science refs check` does **not** honour lifted markers (`fb-2026-07-26-012`), so its marker count runs higher than `validate`'s.
- **`[INACCESSIBLE]` has an acquisition-level bar** (added `task:t130`, 2026-07-26). The t143 *semantics* are unchanged; this is a guard on when you are entitled to apply the token. `[INACCESSIBLE]` means **unavailable after the resolver ladder has been exhausted** — not "one URL or one tool returned 403". Run `science paper-fetch` (Crossref → Unpaywall → arXiv → bioRxiv/medRxiv → Europe PMC → direct OA PDF) before concluding anything is inaccessible; it needs `SCIENCE_CONTACT_EMAIL` set, which `science.yaml` does not configure. Bot-blocks are the specific trap: `medrxiv.org` and `nature.com` return 403 / bounce agents into an auth redirect for papers that are fully open access, and `paper-fetch` walks around both. A near-miss occurred on `paper:Sanford2026`, whose full text the Unpaywall tier retrieved after a direct fetch 403'd. Because a lifted `[INACCESSIBLE]` is a *terminal* verdict, a false one permanently mislabels a reachable source.
- **`status: stub` on a paper entity** means seeded-but-not-yet-read, and is a deliberate project convention outside the toolkit's `(active, retired)` vocabulary. It is suppressed by a value-scoped `health.accepted_validation` entry in `science.yaml`; promoting the paper to a real summary is what clears it, not editing the status.
...
- **D-009:** Vaccine-adverse-event papers — adjudicate on a **trigger × persistence** test (extends D-003); primary scope needs BOTH an infection trigger AND post-acute persistence (≥12-wk **routing convention**, not a universal PAIS threshold — see `topic:pais-case-definition-heterogeneity`), so vaccine papers cap at boundary-monitor regardless. Acute-event vaccine papers (`paper:Nitz2025`: myocarditis/VITT/CVST) are **comparator-only** by vaccine trigger + acute-event estimand (persistence is *unmeasured*, not failed): a **qualitative** infection-vs-vaccination comparator for `question:0081`, never a quantitative anchor, never a PAIS case, never `hypothesis:0001` support. Persistent PACVS (`paper:Bellavite2026`) fails only trigger → boundary-monitor per D-003. Full ruling in `entities/specs/0001-scope-boundaries-for-health-post-acute-infection.md`.
- **D-010:** Scope and evidential admissibility are **separate tests** — primary scope follows infection × persistence; admissibility *for convergence* additionally follows **phenotype × comparator × measurement**. Phenotype-**unmeasured is not phenotype-absent** (indeterminate, not negative), and **upstream mechanism read-across cannot be promoted to evidence of a shared downstream attractor**. Recovered same-trigger controls are the *correct* comparator for `question:0001`'s failed-vs-full-recovery conjunct — never score that as a weakness; a signature that dies against its own healthy-control contrast is the real defect. Route hypotheses correctly: `hypothesis:0001` is broad and **heterogeneously realized** (no shared-analyte requirement); `hypothesis:0012` is the fatigue/cognitive common-path claim — a phenotype mismatch bears on h0012, **not** h0001. `paper:Ramundo2025` / `paper:Sanford2026` stay primary-scope PAIS, supply no fatigue/PEM convergence evidence, and are contextual-but-non-discriminating for h0001.
