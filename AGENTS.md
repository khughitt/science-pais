...
- **D-009:** Vaccine-adverse-event papers — adjudicate on a **trigger × persistence** test (extends D-003); primary scope needs BOTH an infection trigger AND >~12-wk persistence, so vaccine papers cap at boundary-monitor regardless of persistence. Acute vaccine AEs (`paper:Nitz2025`: myocarditis/VITT/CVST) fail both axes → **comparator-only** (antigen-burden asymmetry anchor for `question:0081`; never a PAIS case, never `hypothesis:0001` support). Persistent PACVS (`paper:Bellavite2026`) fails only trigger → boundary-monitor per D-003. Full ruling in `specs/scope-boundaries.md`.
