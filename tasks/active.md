...
VEHICLE HUNT 2026-06-24 (logged in pre-registration:0003 "Screened Vehicles"): no admissible vehicle found; G2 (clean primary-dysautonomia control arm) is the universal blocker. Closest near-miss = van Campen/Visser/Rowe 2025 (PMC12829881), which clears G1 (proximal thigh + distal calf), G3 (long COVID n=143 + ME/CFS n=170), and G4 (SFN ~80-90%) but fails G2 — its comparators are healthy + hEDS, and hEDS is excluded by G2's "no SFN-causing comorbidity" clause (contaminated control). Larsen 2025 LC-POTS deep-phenotyping (medRxiv 2025.04.28.25326587) fails G2+G3 (LC-only, healthy controls only). Actionable: the van Campen group already runs the exact G1/G3/G4 protocol at scale - an admissible vehicle is ~one amendment away (add an idiopathic-POTS/familial-dysautonomia arm). Gibbons-type neuropathic-POTS series (PMC3874039; 38-45% reduced IENFD) can pre-constrain the comparator NLD/lesion rate for the G4 power re-derivation. Task stays BLOCKED - no belief update.
