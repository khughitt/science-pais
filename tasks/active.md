...
- 2026-06-30: Read the two top Core-now papers (Hill2022 N3C/RECOVER, Srivatsan2025 rheumatic-PASC review). Neither reports the t078 estimand (sex x pre-existing-autoimmune -> long COVID, matched, severity+utilization adjusted). Hill2022: best design substrate (N3C, 1:5 matched, computable PASC phenotype) but autoimmune enters as a single POOLED 'rheumatologic disease' Charlson term (OR 1.27), sex is only a covariate (no sex x comorbidity interaction), utilization only a county-level proxy, comparison is PASC-vs-no-PASC among infected (no uninfected arm). Srivatsan2025: pointer-only narrative review, no pooled estimate, sex x ARD never crossed; CRITICAL - in the one matched prospective cohort it cites (Boekel2023) the ARD->PASC association attenuates to NON-SIGNIFICANT after acute-severity adjustment. Conclusion: confirms new-analysis target, not extraction. Severity is plausibly a MEDIATOR (autoimmune->severe acute->PASC, the h0004 path), not just a confounder -> the estimand needs explicit mediation/confounding decomposition. Primary matched cohorts if extraction ever wanted: Boekel2023, Sen/COVAD2023, PatelNJ2024, DiIorio2022.
