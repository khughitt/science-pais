...
- **Date:** 2026-07-19
- **Status:** active
- **Extends:** D-003 (does not supersede) — adds the temporal (acute-vs-persistent) axis and the acute-vaccine cell D-003 never covered. Recorded operationally in `entities/specs/0001-scope-boundaries-for-health-post-acute-infection.md`.
- **Decision:** Admissibility of vaccine-adverse-event papers is decided on **two independent axes**, and **primary scope requires passing both**: (1) **trigger** — infection required (the D-003 rule; vaccination fails it); (2) **persistence** — the phenotype must persist into the post-acute window. There is **no single PAIS duration threshold** (`topic:pais-case-definition-heterogeneity` documents ≥4 wk → ≥3 mo → ≥6 mo, ICC/PICS none); for **routing vaccine papers** this axis uses a **conservative ≥12-week routing convention**, and an **unmeasured** trajectory is **indeterminate**, not a fail. The two axes are not interchangeable: **persistence alone can never promote a vaccine-triggered phenotype to primary scope**, so vaccine papers are capped at **boundary-monitor / read-across** regardless of persistence.
  - **Acute-event vaccine papers (`paper:Nitz2025` — myocarditis/pericarditis, VITT/anti-PF4, CVST) are comparator-only** because of **(i)** the vaccine trigger and **(ii)** an **acute-event estimand** (onset median ≤14 d, range to 90 d; myo/peri time-to-**discharge** 6–7 d; follow-up only 21–183 d, thrombosis resolution not reported). They are **not** classed comparator-only because they were *shown* to fail persistence — the authors state persistence/recurrence/delayed-onset **cannot be assessed**, so this corpus is **indeterminate, not negative, on persistence** (`question:0073`). They are **not a PAIS case, not boundary-monitor-as-a-PAIS-analog, and not independent cross-trigger support for `hypothesis:0001`** (same bar D-003 sets for PACVS/GWS). Admitted **only as comparators**: the infection-vs-vaccination myocarditis asymmetry (vaccination ~2–3.2 vs infection ~15–18.3 — ≈5–9× across non-meta-analyzed ranges) as a **qualitative, hypothesis-generating comparator** (`question:0081`) — **not** a quantitative antigen-burden anchor and **not** belief-bearing support (it compares acute myocarditis, not PAIS incidence, and cannot isolate antigen dose from replication/tropism/non-spike proteins/priming/ascertainment; `proposition:0021` already holds vaccination as triangulation only), plus the acute-vaccine reference point bounding the discriminator D-003 monitors.
...
**Why:**
D-003 fixed the trigger rule and placed PACVS/GWS as boundary-monitor, but never addressed acute-event vaccine papers, whose estimand is onset/incidence rather than a persistent syndrome. `paper:Nitz2025` and `paper:Bellavite2026` both carry scope notes asking for a single adjudication in `entities/specs/0001-scope-boundaries-for-health-post-acute-infection.md`. Separating the temporal axis from the trigger axis routes both cleanly, keeps the (qualitative) comparator value usable without smuggling vaccine data in as PAIS cases, and preserves D-003's bar on counting non-infectious triggers as `hypothesis:0001` support. The persistence axis is a **routing convention, not a universal PAIS threshold**, and the acute-event corpus is treated as **unmeasured**, not resolved.
