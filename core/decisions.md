...
**Why:**
D-003 fixed the trigger rule and placed PACVS/GWS as boundary-monitor, but never addressed acute-event vaccine papers, whose estimand is onset/incidence rather than a persistent syndrome. `paper:Nitz2025` and `paper:Bellavite2026` both carry scope notes asking for a single adjudication in `specs/scope-boundaries.md`. Separating the temporal axis from the trigger axis routes both cleanly, keeps the (qualitative) comparator value usable without smuggling vaccine data in as PAIS cases, and preserves D-003's bar on counting non-infectious triggers as `hypothesis:0001` support. The persistence axis is a **routing convention, not a universal PAIS threshold**, and the acute-event corpus is treated as **unmeasured**, not resolved.
