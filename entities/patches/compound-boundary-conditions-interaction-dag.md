...
inquiry:
  ...
  - ref: unfalsifiable-on-admissible-data
    statement: 'FALSIFIABILITY LIMIT (T1, interpretation:0041; strictly stronger than non-identifiability). unmeasured-shared-confounders (U) points to all four modifiers AND pais-outcome; chronological-age points to all four modifiers AND the outcome; pais-outcome has in-degree 13. Over the MEASURABLE set {age, comorbidity, the four modifiers, acute-severity, outcome} the graph therefore predicts "everything is associated with everything": no conditional independence involving the outcome survives U, and the four modifiers stay mutually dependent even given {age, comorbidity} because U keeps them tied. Every implication that would DISTINGUISH this structure lives in the UNMEASURED layer (immune-homeostatic-reserve, th2-mast-cell-axis, immune-dysregulation, persistent-antigen-fragment-burden, thromboinflammation — all conjectural, source_refs empty). So on any ADMISSIBLE (mediator-unmeasured) cohort the DAG imposes NO refutable constraint — it is UNFALSIFIABLE, which is stronger than the "non-identifiable" framing used elsewhere in this sketch: non-identification says you cannot get a point estimate; unfalsifiability says you cannot even test the drawn structure. TOOLING TRAP (extends formal-checks-are-vacuous): installing pgmpy will NOT fix this. Because export-pgmpy reads the wrong named graph and emits an empty model, a reader who runs `uv add pgmpy` gets GREEN identifiability / adjustment-set checks computed over ZERO edges — worse than the current skip. Two independent reasons no formal identification runs here: (1) the empty-export named-graph bug; (2) pgmpy is not installed, so those checks skip regardless.'
  ...
  - ref: archetypes-markov-equivalent-at-incidence
    statement: 'RESCOPED DISTINGUISHABILITY (T3, interpretation:0041). The claim that the two archetypes "make different, testable structural predictions" holds ONLY at the phenotype-component level, NOT at the incidence scale. Everything that distinguishes shared-bottleneck (Pair 1) from distinct-routes (Pair 2) lives in the UNMEASURED convergence nodes (immune-homeostatic-reserve, th2-mast-cell-axis, immune-dysregulation). On any cohort lacking those measurements — which cohort-design-requirements concedes is every admissible cohort — the two wirings imply the SAME conditional independencies over the measurables and are MARKOV-INDISTINGUISHABLE at the incidence level. The genuine distinguishing signal is the PHENOTYPE-COMPONENT VECTOR: Pair 2 predicts opposite-signed components (Th2/mast-cell and thrombovascular up, Th1/neuro-cognitive-fatigue down), Pair 1 predicts same-signed amplification — outcome-observable, but ONLY with a phenotype-resolved outcome AND enough doubly-exposed cells, the exact resource D-004 blocks. So "different testable predictions" is true only in a data regime that does not currently exist; the incidence-scale archetype contrast is UNTESTABLE and the distinguishing claim is scoped to the phenotype-vector level. See phenotype-redirection-needs-resolved-outcome.'
...
> compound-modifier analysis would have to be set up**, not a validated causal model. Stronger still
> **(T1)**: on any **admissible (mediator-unmeasured)** cohort the DAG imposes **no refutable constraint
> at all** — it is **unfalsifiable**, not merely unidentified (see `unfalsifiable-on-admissible-data`),
> and installing `pgmpy` would yield *green checks over an empty graph*, not a real identification result.
...
convergence nodes unmeasured the two archetypes are **Markov-indistinguishable on incidence**, so the
...
| Immune homeostatic reserve | **Hypothesized** shared mediator (bottleneck) — **in-degree-4 exposure-induced collider** of both Pair-1 modifiers | Do **not** adjust and **do not stratify**; a mediation sub-test needs **g-formula / natural-effects**, not conditioning (T2) |
...
- **Mediators** (reserve, antigen burden, dysregulation, Th2/mast-cell, thrombo, severity) are
  ...
  mediation or severity-controlled sub-question needs a **g-formula / natural-effects** estimator for
  exposure-induced mediator–outcome confounding (with interaction-specific sensitivity), not
  conditioning; the path Hammel2023 left open is fixed by **modelling** severity, not stratifying on it.
...
- **Critique fixes applied (t141, 2026-07-19; interpretation:0041).** All six items addressed in this
  ...
  severity are flagged as in-degree-4 exposure-induced colliders and any valid sub-test now requires
  g-formula / natural-effects, reconciling `mediators-not-conditioned-for-joint-effect` with
  `compound-selection-collider` (the T2 contradiction is closed). **(T1, done)** added
  `unfalsifiable-on-admissible-data` (no refutable constraint on any admissible cohort; the
  pgmpy-green-over-empty-graph trap). **(T3, done)** added `archetypes-markov-equivalent-at-incidence`
  and scoped the distinguishability claim to the phenotype-vector. **(M1, done)** added the load-bearing
