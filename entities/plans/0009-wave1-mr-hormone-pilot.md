...
Estimators: **naive IVW / MR-Egger / weighted-median** (log-OR outcome) as the
uncorrected comparator, then **MRlap** (Mounier & Kutalik 2023) for joint
sample-overlap + weak-instrument + winner's-curse correction — **mandatory here**
because Ruth is 100% UK Biobank and the HGI outcome pools UKB (structural overlap).
MRlap is applied per `plan:0008` **KD-scale contract**: genome-wide munged sumstats
restricted to HapMap3, `eur_w_ld_chr` LD-score reference, **total N** (observed
scale for the binary outcome, not effective N), and the MRlap effect reported in its
**native standardised/observed scale beside** the naive log-OR IVW — the two are
**not** merged as one like-for-like estimate. MRlap is **pinned by git commit** (no
tagged release). Weighted-median SE is bootstrapped → **fixed RNG seed** recorded.
Reproducibility: extend the `plan:0007` conda-lock env with `r-mrlap` (pinned) + a
locked `ldsc.yaml`; run under Snakemake `--use-conda`. Payloads gitignored under
`data/`; outputs + manifest under `results/wave1-mr-hormone-pilot/`.
...
- **MRlap infrastructure (new, in-scope per D-006)** — `eur_w_ld_chr` EUR LD-score
  reference + the HapMap3 SNP list; staged checksummed at Task 1 as a tracked
  reference dataset (entity created there under the retrieval-probe exception).
- `TwoSampleMR`, `MRlap`, `MendelianRandomization`, LDSC/munge tooling, local `plink`.
...
1. **Stage inputs + infrastructure.** Retrieve the six Ruth strata + `GCST90454541`
   (harmonised `fullPvalueSet`) into `data/raw/gwas/`; stage `eur_w_ld_chr` +
   HapMap3 and the 1000G-EUR panel. Record SHA-256 / build / rows for each; resolve
   the Ruth entity's assembly label and upgrade its `verification_method`; create the
   LDSC-reference dataset entity; rerun the data-access gate. Confirm the LD-score /
   HapMap3 build+ancestry reconcile (rsID) with the GRCh37-native Ruth and the
   GRCh38 outcome.
...
3. **Naive MR (comparator).** Harmonise (`action = 2`) and run IVW / MR-Egger /
   weighted-median (log-OR outcome; seeded weighted-median) for each trait × stratum;
   report per/mean F, Egger intercept, and IVW/Egger/WM concordance. Stream the
   outcome extraction by rsID (record peak memory + wall-clock).
4. **MRlap overlap correction.** Munge each exposure + the outcome genome-wide to
   HapMap3, run MRlap with `eur_w_ld_chr` per the KD-scale contract; record the
   **estimated UKB overlap fraction** and the corrected effect in its **native
   scale**, beside the naive IVW. **Scale/resource validation on real data:** run the
   full genome-wide munge + MRlap on the staged sumstats with **peak memory +
   wall-clock recorded** (the new, untested resource step). Label the scale of every
   estimate.
5. **Write-up + go/no-go.** One results note: the mechanics outcome, the exploratory
   ...
   sex-modification claim). Recommend whether the deferred `plan:0008` full design is
   worth resuming if/when a matched EUR outcome appears, and whether the MRlap stack
   is sound. Emit the reproducible bundle under `results/wave1-mr-hormone-pilot/`
   (datapackage.json with entity cross-refs + provenance DAG; qa_report.{json,md}
   with structural hard-stops incl. overlap fraction + scale labels + resource;
   run_metadata.json with seeds/versions/pinned-MRlap-commit/SHA-256s; env locks).
...
- **MRlap runs**, records the UKB overlap fraction, and returns a corrected estimate
  in a **clearly labelled native scale** (not merged with the log-OR IVW); the
  genome-wide munge + MRlap resource cost is recorded.
...
- Per-file SHA-256 / build / row counts recorded; Ruth assembly resolved; LDSC-ref
  entity created and gate-passing.
...
- MRlap overlap fraction, corrected estimate, and its **native scale label** present;
  no cross-scale comparison presented as like-for-like; genome-wide resource
  (peak mem + wall-clock) recorded.
