...
| **Vascular-thromboinflammatory** | **MALE-biased (reversed)** for hard endpoints — VTE RR 1.43 (1.19–1.71) male; post-COVID CV mortality HR 1.33 (1.01–1.74) male; endothelial dysfunction (FMD) worse in males — opposite the overall female PAIS skew | hard clinical endpoint | Abubasheer2025 (VTE meta); CV-mortality cohort; Ambrosino2021 (FMD) | none |
...
- **Severity confounding** drives the male-biased vascular signal in the opposite direction:
  males had more severe acute COVID, which independently raises thrombotic risk, so the
  "male vascular" reversal is partly acute-severity carryover, not a clean long-COVID sex effect.
...
- **Does the male-biased vascular domain survive severity adjustment?** (P2, methodological) If it
  persists within acute-severity strata, it is a genuine domain reversal; if not, it is carryover.
...
(not minted) pending the severity-adjustment question below.

- **Promoted the two best-supported subphenotype findings to propositions + evidence-lines**
  (done above; the third — vascular — held):
