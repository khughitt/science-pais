...
related:
  ...
  - dataset:emexp2069-gulf-war-illness-pbmc
created: "2026-07-09"
updated: "2026-07-10"
...
> **Update (2026-07-10) — 3rd non-infectious column built (`E-MEXP-2069` GWI PBMC), reverse projection FULL-RANK:** The compartment-matched GWI column (9 GWI / 11 controls, PBMC, HG-U133 Plus 2.0, baseline arm; built via raw CEL → pure-R RMA because affy/preprocessCore threaded C fails on this host, → 20,338 genes) resolves the 2-column under-resolution: with 3 non-infectious columns `r_eff = min(R=2, n_noninf−1=2) = 2 = PAIS R` (`identifiability_pass=true`). **Reverse result:** verdict **`noninfectious_axis_not_reproducible_indeterminate`** — the leave-one-non-infectious-out ceiling is **LOW (0.053)**, so even across FM *and* GWI the non-infectious case-vs-control axis does **not** reproduce; PAIS recovers U (0.072) *above* the ceiling (ratio 1.37), an incoherent "manifold" comparison. There is no reproducible generic-non-infectious axis for PAIS to be tested against, so the read-across cannot become `validated_specificity` by this route. **The 3rd column also DISENTANGLES the forward confound named in the 2026-07-09b update:** GWI is **PBMC** (compartment-matched to the flagship) yet recovers **0.213** — like the WB-microarray FM (0.234) and ~5× the PBMC-RNAseq FM flagship (0.045). Holding compartment fixed (PBMC) and a tiny N (9v11), the ~5× jump therefore tracks **PLATFORM (microarray vs RNA-seq), NOT compartment/blood-composition and NOT condition** — a *technical* (platform/enrichment-structure) confound on forward recovery (microarray mean 0.224 vs RNA-seq 0.045). Net: the reverse test is now full-rank and returns a clean **negative/indeterminate** (no reproducible non-infectious axis), and the forward "recovery" is explained by platform, not biology — both reinforce `exploratory_flagship`, and neither licenses a specificity claim. Related: [[dataset:emexp2069-gulf-war-illness-pbmc]].
