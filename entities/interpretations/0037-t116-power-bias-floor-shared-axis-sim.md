...
- **A *shared* (correlated) arm-bias artifact defeats the test — the harmonization confound.** The surface models arm bias as *independent* per arm (the error harmonization reduces). But running one harmonized protocol can *introduce* bias correlated **across** arms (a common platform / pipeline / reference-set axis on every arm). A probe adds such a shared axis with **no genuine attractor**, at the R=2 / K=6 / N=30 cell that *does* arbitrate against `question:0017` (true-attractor arbitrating power 0.89). As the shared artifact grows from 0 → the true signal strength (β = 0 → λ), the off-diagonal SD collapses (0.136 → 0.082) and the **false-"it's-an-attractor" rate climbs 0.13 → 0.30 → 0.62 → 0.92** — i.e. a fully shared artifact is, to this SD-of-off-diagonals statistic, **indistinguishable from a genuine attractor**. The structural test controls the false rate only against the *heterogeneous* `question:0017` null; it has **no protection** against a *homogeneous* shared artifact. Consequence for `question:0050`: harmonization must control **correlated** bias, not merely its magnitude — full-recovery-control contrasts and explicit shared-artifact diagnostics (e.g. negative-control features, cross-arm batch structure) are **load-bearing, not optional**. *(discriminating; identified in the t116 pipeline review, finding #1)*
...
- **Determinism:** fixed seed 1729 (echoing the t035 determinism seed), numpy PCG64 pinned to 2.4.6, 8000 reps/cell; re-running reproduces the surface. The sim is now a config-driven Snakemake workflow (`code/workflows/t116-power-bias-floor/`) that emits a `datapackage.json` manifest alongside the surface.
...
- **Null construction verified + now asserted:** the `question:0017` null is concordance-matched per cell by binary search on its loading scale; realized mean concordance tracks `hypothesis:0001`'s (~0.43–0.53). A per-cell **fail-early assertion** (added per the pipeline review, finding #2) now hard-stops the run if the match error exceeds 0.03 (i.e. if the search pins to the loading-search boundary and returns an unmatched null); observed max match error is **0.0048**, so every reported cell is genuinely matched — the two hypotheses are separated *only* by structure, not by average overlap.
...
- **Arm-specific bias magnitude is fixed in the main surface**, which assumes a moderate, non-catastrophic residual arm bias — i.e. that harmonization *works* to the degree `interpretation:0036` requires. The pipeline-review shared-bias probe now tests the *direction* of that assumption (independent vs correlated bias) and finds the correlated case is not merely a magnitude to lower but a **qualitatively different failure that the test cannot detect** (see Findings). What remains un-swept is the *magnitude* of independent bias (α) — a straightforward further extension — and the joint independent-×-correlated bias surface.
