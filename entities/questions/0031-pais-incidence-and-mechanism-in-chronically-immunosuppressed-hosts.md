...
lens_views:
- lens: population
  rationale: 'PAIS mechanisms assume an immune system able to mount, sustain, then fail to resolve inflammation. Immunosuppressed hosts are a natural experiment dissociating candidate mechanisms: if antigen persistence/activation drive PAIS, partial suppression might reduce incidence; if exhaustion/dysregulation drive it, baseline dysfunction might amplify risk. The biologic-DMARD population is essentially unstudied for post-infectious sequelae.
