...
- **A second data type reinforces the narrow thesis (`interpretation:0043` / t127).** Nilforoshan2026's test-based claims design (PCR+ vs PCR− + negative-control outcomes, run on a subset of the 244.7M-patient Komodo database) drops the **negative-control-outcome** false-positive rate from 53.1% → 4.1% by ascertainment control — showing a large share of a *conventional* claims analysis's apparent signal is ascertainment-driven. **Precision:** 53.1%/4.1% are association rates on biologically-implausible negative controls (a **bias metric, not long-COVID burden**; the 4.1% residual is protective-direction). **This does not convert the ~10–20% convergence into "artifact":** Nilforoshan measures *attributable ICD-outcome counts* (low, unquantified sensitivity to the subjective fatigue/PEM/cognitive cluster; RR ≥ 1.1 floor) — a **non-comparable estimand, channel, and era** from RECOVER's PRO syndrome prevalence — so it cannot directly deflate the 10% figure. (RECOVER's own 23% → 10% drop is **not** clean selection deflation either: the 10% subset changes variant era, enrollment timing, and prior immunity, not selection alone.) The ascertainment contribution is real in direction but its **magnitude is unidentified** — the same "different estimands cannot be combined" discipline this question applies to Dubbo vs Ballouz.
