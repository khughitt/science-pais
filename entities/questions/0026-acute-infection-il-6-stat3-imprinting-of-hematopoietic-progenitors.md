...
- 2026-07-10: **That direct route is now feasibility-scoped (t107 → `interpretation:0040-t107-hspc-epigenomics-feasibility-banked-pbmc`), and the pivot lands on *this* question's effector locus.** The feasible opportunistic readout on banked PAIS PBMC is **monocyte-progeny (CD14⁺) ATAC-seq** — i.e. exactly the hyperreactive-monocyte imprint q0026 predicts — because monocytes are abundant (~10–20% of PBMC, all-platform) whereas circulating CD34⁺ HSPCs are too rare (~0.05%) for single-vial single-cell ATAC and are compartment-mismatched to Cheong2023's marrow HSPCs. So the q0026 effector-state prediction is the cheaply- and directly-testable one (severity-stratified, on RECOVER / UK ME/CFS Biobank banked PBMC); the upstream HSPC-source localization needs LIINC banked bone marrow.