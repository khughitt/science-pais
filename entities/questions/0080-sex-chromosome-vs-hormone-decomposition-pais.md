...
- **Decomposition finding logged (t125, `interpretation:0042`, 2026-07-19):** A re-read of `paper:Chaulagain2026` for this decomposition concludes the review **juxtaposes but does not isolate** the two axes. Its "limited evidence supporting a role for gonadal steroids in PASC outcomes so far" is an **evidence gap**, not positive evidence that the X-chromosome-dosage axis carries the female excess; the chromosome-linked findings it cites are heterogeneous and mostly not PASC-cohort-specific (XIST is an *acute-phase* predictor in females who later developed LC — Hamlin 2025, n=45; TLR7 biallelic is general sex-immunology; male Y-linked genes show reduced *expression*, not genomic loss, and are not linked to a cardiovascular phenotype in the primary source). So `proposition:0044` is a **salient weak rival**, not raised to parity. The axes are **not orthogonal and not separable by a single mediation split of the total sex effect** (chromosome complement shapes gonadal development; steroids regulate X-linked genes, e.g. estrogen→TLR7); the identifiable targets are conditional estimands — hormone-state contrasts *within* chromosome strata, chromosome-dosage contrasts *under comparable hormone exposure*, and their interaction. On the hormone-liability limb specifically, the MR route (`dataset:ruth-2020-shbg-testosterone-gwas` → `dataset:covid19-hgi-longcovid-gwas`) is **not a candidate — it was D-005-authorized and already executed** (`plan:0009`), returning a **null/uninformative** result under ancestry (KD1) and mixed-sex-outcome (KD3) ceilings; a matched EUR-only + sex-stratified HGI rerun is in-scope D-006 maintenance, and only a distinct outcome vehicle (FinnGen/non-HGI) needs a fresh scope decision. The full multi-estimand decomposition still has no admissible vehicle.
...
- Priority level: High for `hypothesis:0005` — this question must be answered (or the gap formally acknowledged) before the **hormone-mechanism leg (`proposition:0002`) can be strengthened/promoted beyond its current fragile status**. (h0005 itself is already `active`; the block is on the fragile hormone leg, not on the hypothesis's activation.) No admissible human PAIS vehicle exists for the full karyotype-and-hormone-resolved decomposition; the hormone-liability MR limb alone has been run (`plan:0009`) and is null under ancestry/sex-outcome ceilings.
