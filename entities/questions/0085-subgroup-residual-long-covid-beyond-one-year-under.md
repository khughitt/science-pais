...
Nilforoshan2026 finds zero attributable long COVID outcomes clearing its compound RR ≥ 1.1 + Bonferroni threshold at 360–720 days (days 12–24 months post-infection) under their test-based prospective design, run on the test-based analytic cohort (a subset of the 244.7M-patient Komodo database, not all 244.7M). This is a **whole-cohort (unstratified)** result with a conservative detection threshold and an author-estimated ~10–20% possible negative attenuation; the authors gloss it as population health "returning to baseline," but subgroup sensitivity is unknown. The null could therefore fail to resolve genuine persistent effects in specific subgroups: hospitalized / severely ill patients, immunocompromised individuals, those with specific comorbidities, unvaccinated patients, or those with early-wave (Delta / pre-Omicron) infections. The question asks: when the ascertainment-correction logic of Nilforoshan2026 is applied within specific strata, which subgroups retain statistically and clinically meaningful attributable effects beyond one year?

## Why It Matters

- Directly determines the validity of hypothesis:0004 (acute severity threshold predicts self-sustaining PAIS): if the whole-cohort result shows no attributable outcomes by 1 year but hospitalized patients do, the threshold model is supported.
...
- The whole-cohort null (zero outcomes clearing the threshold) at 360–720 days is most plausibly correct for the broad PCR-tested population as an unstratified estimate. It is not inconsistent with a small (<5%) subgroup of severe-acute-illness patients who remain symptomatic at 1–2 years — such a tail would be **under-detected in a whole-cohort estimate (subgroup sensitivity unknown), not shown absent**, and could still be clinically significant.
...
- The author-estimated ~10–20% possible attenuation in Nilforoshan2026 means an effect of RR 1.05–1.1 in the full cohort could plausibly be real but missed (this is a possibility the authors flag, not a correction factor to subtract). In a severity-enriched subgroup (higher baseline rate of the outcome), the same absolute excess would manifest as a higher RR that crosses the detection threshold — which is exactly why the whole-cohort null does not establish subgroup absence.
