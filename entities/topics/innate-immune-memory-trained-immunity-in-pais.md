...
- NLRP3 is epigenetically primed in trained monocytes: H3K4me3 at the NLRP3 and pro-IL-1β loci is elevated in trained cells, making them more responsive to inflammasome activation without de novo transcriptional priming (field literature).
...
SARS-CoV-2 releases mitochondrial DNA (mtDNA), viral RNA fragments, and micronuclear dsDNA into the cytoplasm; herpesviruses and Borrelia generate cytoplasmic nucleic acids via DNA damage or lytic replication. cGAS detects cytoplasmic dsDNA → activates STING → TBK1 → IRF3 → IFN-I production. This is the sensor that initiates and sustains IFN-I signaling without requiring active replication (`question:0023`). The trained-immunity connection: (1) STING activation can epigenomically prime innate cells — chronic low-level STING signaling may deposit chromatin accessibility at IFN-stimulated-gene loci, creating a form of IFN-trained innate memory; (2) conversely, trained HSPCs arriving from bone marrow may have elevated STING pathway accessibility and thus mount stronger IFN-I responses to each new DAMP or cytoplasmic-DNA signal. The precise relationship — whether cGAS-STING is upstream of HSPC training (as a seeding signal) or downstream (as a trained-cell output, amplified by the imprint) — is not established in PAIS. In the context of q0023, training offers a mechanism by which cGAS-STING signaling could sustain IFN-I *without* ongoing cGAS ligand: the chromatin imprint keeps IFN gene loci accessible even when the initial dsDNA stimulus is cleared.

### NLRP3/GSDMD pyroptosis loop (question:0024)

Trained monocytes and macrophages are constitutively primed for NLRP3 inflammasome activation: H3K4me3 at the NLRP3 and pro-IL-1β promoters is elevated in trained cells, eliminating the requirement for NF-κB-mediated transcriptional priming before NLRP3 activation. This means each small secondary stimulus (DAMPs, ATP, cholesterol crystals, urate) triggers a disproportionate IL-1β/IL-18 response in trained cells — matching the self-amplifying loop described in `question:0024`. Gasdermin-D pores releasing pyroptotic contents (IL-1α, ATP, mtDNA, HMGB1) then constitute a secondary DAMP signal that could re-train neighboring cells or re-engage cGAS-STING. The connection to NLRP3 in the PAIS-trained-immunity frame is thus a **positive-feedback loop**: trained NLRP3 hyperreactivity → more pyroptotic DAMP release → DAMP-driven re-training of nearby cells or fresh monocyte recruits. This is mechanistically coherent but remains uninvestigated as a DAMP-driven training loop in any PAIS system. A 2025 Frontiers Immunology review on DAMP-driven trained immunity in critical illness is beginning to formalize this connection.
...
A plausible integrated model: (1) Severe acute infection → high IL-6 → HSPC STAT3 signaling → HSPC epigenomic imprinting → continuous hyperreactive monocyte output for months-to-years (the central trained-immunity axis, q0026). (2) Each newly arriving trained monocyte has elevated H3K4me3 at cGAS/STING/IFN loci (q0023 axis) and NLRP3/pro-IL-1β loci (q0024 axis), making it hyperresponsive to any residual DAMP signal. (3) Pyroptotic GSDMD pores from NLRP3-hyperresponsive trained cells release mtDNA and other DAMPs → re-engages cGAS-STING in neighboring cells → sustains IFN-I production → feeds back onto HSPC differentiation signals. This creates an antigen-independent self-sustaining loop that could maintain the h0001 attractor state. All connections are mechanistically coherent but remain hypothetical in PAIS; no study has tested them in an integrated fashion.
