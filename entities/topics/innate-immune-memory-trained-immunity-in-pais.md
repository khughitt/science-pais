...
**Tolerance vs training: the post-sepsis dual phenotype.** Sepsis and critical illness can produce *either* trained immunity (enhanced reactivity, hyperinflammatory) *or* immune tolerance (endotoxin tolerance, immunosuppression, vulnerability to secondary infection) in myeloid cells, depending on stimulus intensity, timing, and cell compartment. Bomans et al. 2018 (Frontiers Immunology), a mouse-only cecal-ligation-and-puncture study, contributes only the *training* side of this dichotomy: isolated bone-marrow monocytes from sepsis survivors were hyperreactive (elevated TNF/IL-6 on ex-vivo LPS restimulation) with enhanced basal glycolysis, alongside expansion of the granulocyte-monocyte progenitor (GMP) compartment — while the *systemic* whole-blood cytokine response was **unchanged**. Bomans did **not** demonstrate peripheral-monocyte tolerance, and did not assay progenitor chromatin (RNA-seq on BM monocytes only), so it is not itself evidence of a coexisting tolerance arm or of HSPC-level epigenomic reprogramming. The dual-phenotype conjecture — that in PAIS peripheral circulating monocytes may be tolerized (blunted TLR response, feeding immune exhaustion in `hypothesis:0003`) while marrow-derived naive progeny are trained — is drawn from the broader endotoxin-tolerance literature, not from Bomans, and is not well-characterized in any PAIS cohort [@Bomans2018].
...
- Durable training requires HSPC-level imprinting: Mitroulis2018 (Cell), a mouse-only study, demonstrated that β-glucan modulates myeloid progenitor fate in bone marrow and that this bone-marrow effect — not just peripheral monocyte memory — accounts for the multi-month persistence of trained immunity. The decisive experiment transferred *sorted LT-HSCs* from trained donors into naive recipients; the required paracrine signals were **IL-1β and GM-CSF** (IL-6 was *not* altered), and readouts were functional/transcriptomic/metabolomic (no progenitor chromatin assay).
...
- **Bomans2018 (Frontiers Immunology).** The post-sepsis parallel: Bomans, Schenz et al. 2018, in a **mouse-only** cecal-ligation-and-puncture model (no human arm), showed that sepsis induces long-lasting enhanced cytokine production (elevated TNF/IL-6 on ex-vivo LPS restimulation) and enhanced basal glycolysis in *isolated bone-marrow monocytes*, together with GMP expansion — while the systemic whole-blood cytokine response was unchanged. It is suggestive that a critical-illness stimulus can shift the marrow myeloid compartment toward a trained state, but it did not assay progenitor chromatin (RNA-seq on BM monocytes only), so "central training" here is inferred from GMP expansion rather than demonstrated at the HSPC epigenomic level. It is the principal (indirect) support for the claim that non-COVID PAIS triggers can drive central training; whether Borrelia, Coxiella, or EBV acute infection similarly imprints HSPCs is entirely inferred [@Bomans2018].
...
- **Non-COVID PAIS.** HSPC-level *epigenomic* reprogramming has been documented in a human study only after COVID (Cheong2023); the non-COVID parallel (sepsis, Bomans2018) is a **mouse** model showing trained marrow monocytes and GMP expansion but no progenitor chromatin assay. Whether Borrelia, Coxiella burnetti, EBV, dengue, or influenza acute infection drives comparable HSPC central training in humans is unknown. The ME/CFS advocacy in Humer2025 is conceptual, not backed by ME/CFS-specific epigenomic data.
- **Tolerance vs training coexistence.** Post-COVID monocytes show both activated and tolerized features depending on cohort, timing, and assay. A compartment-divergent model (HSPC/marrow-level training coexisting with peripheral tolerance) is a plausible reconciliation but is **not** established by Bomans2018 (which showed only the training arm, with an unchanged systemic response, in mice); whether a circulating-monocyte tolerance observation contradicts or coexists with central HSPC training has not been directly tested in long COVID or ME/CFS.
...
- **Bomans2018** — Bomans K et al., Front Immunol 2018; mouse CLP model — sepsis induces long-lasting trained immunity in bone-marrow monocytes plus GMP expansion (systemic response unchanged; no progenitor epigenomics); post-sepsis parallel for non-COVID PAIS training, at the marrow-monocyte rather than HSPC-epigenomic level.
