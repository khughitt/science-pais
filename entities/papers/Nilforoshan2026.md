...
**No attributable outcomes at 360–720 days.** No ICD outcome clears the compound RR ≥ 1.1 + Bonferroni threshold at 360–720 days; the authors characterize this as average population-level health "returning to baseline" within approximately one year (an interpretation of the zero-count on this outcome set, not a measured average-health trajectory, and not a statement about every subgroup). This contrasts with conventional analyses of the same data finding 369 persistent significant effects. The authors note that symptoms still occurring at >1 year in COVID-19 patients are not demonstrably specific to COVID-19 — they may reflect non-COVID conditions that originally prompted the healthcare visit and PCR test.
...
**Relevance to hypothesis:0010 (slow recovery gradient, not a stable chronic state).** The whole-cohort zero-count by one year under the corrected design is **consistent with (not confirmatory of)** h0010's claim that the "chronic state" in many PAIS studies is partly a measurement artifact of insufficient follow-up and control-group mismatch. The design's RR ≥ 1.1 threshold is conservative: the authors note that mild effects (RR < 1.1) may persist undetected, and subgroup sensitivity is unquantified. The five outcomes persisting to 120–360 days (myopathy, hair loss, pulmonary embolism, acute PE, respiratory failure) are consistent with a true subset of longer-duration effects, compatible with h0010's prediction of trajectory heterogeneity — but "compatible with" is the correct strength, since a claims-count design and RECOVER's trajectory classes measure different constructs (see `interpretation:0043`).

**Relevance to hypothesis:0004 (acute severity threshold).** The paper does not stratify results by acute illness severity, so it cannot directly test h0004. However, the 30–120 day outcomes dominated by acute respiratory and cardiovascular sequelae (pneumonia, pulmonary embolism, respiratory failure, shock) are consistent with the view that patients who crossed an acute severity threshold drive the detected signal disproportionately. The whole-cohort zero-count by 1 year is also consistent with most patients *not* crossing h0004's threshold into a self-sustaining state (an ecological inference, since the paper does not stratify by severity).
...
- **Stratified analysis by acute severity and vaccination status.** The most important gap: do the 360–720 day null results hold for hospitalized, unvaccinated, or immunocompromised subgroups? If a small subgroup (e.g., ~5%) retains attributable effects, it would be **under-detected in the whole-cohort estimate (subgroup sensitivity unknown), not shown absent** — clinically significant yet unresolved by an unstratified count. Relates to hypothesis:0004 and `question:0085`.
...
- **Reconcile with clinical cohort studies.** How do these estimates compare to RECOVER, Thaweethai2023, and other prospective clinical cohorts that used different ascertainment methods and found higher long-COVID fractions? A head-to-head design-sensitivity analysis across datasets is needed. **DONE (`interpretation:0043` / t127, 2026-07-19):** the "order-of-magnitude smaller" gap vs RECOVER (`paper:Thaweethai2023`, 10% PASC+ at 6 mo) is **not directly interpretable** — the two use different estimands, channels, thresholds, eras, and cohorts, so it is neither a factual contradiction nor a clean "X% artifact." This design counts *attributable ICD outcomes* (low, unquantified sensitivity to RECOVER's PRO fatigue/PEM/cognitive cluster; RR ≥ 1.1 floor); RECOVER measures a self-reported syndrome prevalence. On persistence the zero-outcome result is **uninformative about** (not proven blind to) RECOVER's ~5% persistent-high trajectory class (`cite:Thaweethai2025`); the severity-enriched VA 3-year cohort (`paper:Cai2024`) is *consistent with* a severity-threshold reading but the three-study inference is ecological/underidentified. Ascertainment likely contributes but its magnitude is unidentified; `question:0085` stays open — none of the three adjudicates it.
