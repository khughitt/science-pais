...
- **Cross-design reconciliation with test-based claims estimates (`interpretation:0043` / t127, 2026-07-19):** this cohort's PRO syndrome prevalence (10% Omicron/≤30-day subset; 23% full sample; 3.7% score-positivity among uninfected volunteers — an observed rate, not a causal floor) and Nilforoshan2026's near-null *attributable-ICD-outcome* counts (`paper:Nilforoshan2026`) are **non-comparable** — different estimands, channels (self-reported fatigue/PEM/cognitive syndrome vs billing-coded hard events at RR ≥ 1.1), and variant eras — so the apparent order-of-magnitude gap is not directly interpretable, not a contradiction. The 23% → 10% drop is **not** clean selection deflation (it also changes era/timing/immunity). On persistence the two are compatible; the residual subjective tail (RECOVER's ~5% persistent-high class, `cite:Thaweethai2025`) is `question:0085`.
